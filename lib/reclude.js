...
module.exports.act = function(buffer, encoding, callback) {
        ...
        if(!module) {
            ...
            if(!module.buffer) {
                boneFs.readFile(module.path, function(error, buffer) {
                    if(error) {
                        bone.log.warn('bone-act-reclude', [error, '. File: ', module.path].join(''));
    ...
    var walkman = function(buffer, modulePath, name) {
        ...
        try {
            ...
            bone.log.warn('bone-act-reclude', [e, '. File:', modulePath].join(''));
    ...
    if(options.common) {
        // todo mulit file check
        var commonPath = boneFs.pathResolve(options.common);
        if(boneFs.existFile(commonPath)) {
            bone.utils.fs.dependentFile(commonPath, function(error, traceList) {
                if(traceList) {
                    traceList.forEach(function(file) {
                        dependList[file] = true;
                    });
                }
                transformTree();
            });
        } else {
            bone.log.warn('bone-act-reclude', `common file "${options.common}" not exist!`);
            transformTree();
        }
    } else {
        transformTree();
